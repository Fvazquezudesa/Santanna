/*==============================================================================
  PROCREAR — Paper Tables: Labor Market Outcomes (SIPA)

  THIS SCRIPT GENERATES THE OFFICIAL PAPER TABLES FOR LABOR OUTCOMES.
  Lean version: only the regressions whose output is \input'd by paper.tex.
...
* =============================================================================
* USER CONFIG — edit these when running on a different computer or dataset
* =============================================================================

* Folder containing the input datasets (Data_SIPA.dta, Data_sorteos.dta,
* Inflacion_desde_Ene2007.dta, ...)
global data "/Users/francomartinvazquez/Dropbox (Personal)/Procrear Santanna/DATA"

* Folder where output tables are written. Tables go to "$output/tables".
...
* Name of the nominal monthly wage column in the SIPA-like panel.
* In Data_SIPA.dta it is "remuneracion"; other bases may use a different name.
local wage_var "remuneracion"

* Name of the integer date column (YYYYMM format) in the SIPA-like panel.
* In Data_SIPA.dta it is "mes". Script internally renames it to "mes".
local fecha_var "mes"

* Name of the female indicator column (0/1) in the SIPA-like panel.
* In Data_SIPA.dta it is "mujer". Script internally renames it to "mujer".
...
* Deseasonalize (aguinaldo adjustment: June/December ÷ 1.5)
